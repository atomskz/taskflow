...
```
pages/ + components/      ← презентация (JSX + классы из styles.css; dynamic-стили inline)
        │  useApp()
        ▼
...
- **Стили.** Inline-CSS в JSX полностью убран. Дизайн-токены, ресеты и keyframes — в `index.css`; общие переиспользуемые классы (кнопки, карточки, инпуты, бейджи, навигация, модалки, тосты, чипы, формы и т.д.) — в `client/src/styles.css`; у каждого компонента/страницы свой co-located CSS-файл (`AppLayout.css`, `DashboardPage.css`, `CalendarPage.css` …), импортируемый рядом с компонентом. Все hover/focus — реальными CSS `:hover`/`:focus` (старая JS-обёртка `<Hov>` и хелперы `css()/mix()` удалены). Атрибут `style={...}` остаётся только для **динамических, data-driven** значений — как правило через CSS-переменные (напр. `style={{'--c': vm.priColor}}` → `background: var(--c)`): цвета приоритетов/статусов, вычисляемая геометрия графиков, цвета меток календаря.
