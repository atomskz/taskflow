...
| `npm test`            | Все тесты: API (server) + клиентские unit             |
| `npm run test:e2e`    | End-to-end (Playwright; нужен `npx playwright install`)|

## Тесты

```bash
npm test            # бэкенд (API) + клиентские unit-тесты
npm run test:e2e    # сквозной сценарий через Playwright
```

- **API** (`server/test/api.test.js`) — на изолированной временной БД: регистрация/валидация, дубликат email, неверный пароль, защита маршрутов, CRUD задачи, серверные фильтры/пагинация, агрегаты дашборда, refresh-токены/отзыв, мягкое удаление/восстановление, security-заголовки, rate limiting.
- **Клиентские unit** (`client/src/lib/__tests__/`) — чистые селекторы: `isOverdue`, `filterSortTasks`, `buildDashboard`, `buildCalendar`.
- **E2E** (`e2e/`) — регистрация → создание задачи → завершение → дашборд. Перед первым запуском: `npm install && npx playwright install chromium`.

## Известные ограничения

- Тёмная тема пока не покрывает несколько декоративных градиентов на лендинге/странице входа (они намеренно остаются тёмно-цветными).
- БД — SQLite в одном процессе; in-memory rate limiter не разделяется между инстансами. Для горизонтального масштабирования нужен общий стор.
